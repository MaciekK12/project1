...
const AddArenaBtn = ({id}) => {
  ...
  const pokemonFightStatus = useSelector(state => state.ArenaPokemon.fightDone)
  const isInArena =  pokemonArray.includes(id)

  const handleClick = () => {
    if (pokemonFightStatus === true) {
        alert('Najpierw zakończ walkę pomiędzy pokemonami poprzez naciśnięcie "OPUŚĆ ARENĘ"')
    } else {
